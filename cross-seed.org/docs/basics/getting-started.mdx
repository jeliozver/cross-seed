...
**Use [npm installation](#native-installation-with-npm)** if your torrent client
is running on Windows. Do not use Docker if you want linking, partial matching,
or data-based matching. See the [Windows quickstart](./windows.md) for a full
Windows setup.
...
For common errors, see [Common Setup Failures](./common-setup-failures.md) and
[FAQ and Troubleshooting](./faq-troubleshooting.md) pages.
...
- [**Managing the daemon**](./managing-the-daemon) with Docker or systemd for
  long-term use.
- [**Common setup failures**](./common-setup-failures.md) for Docker paths,
  hardlinks, Torznab URLs, networking, permissions, and recovery.
